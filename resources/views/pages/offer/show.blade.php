                ...
                <div class="col-12">
                    <div class="card">
                        ...
                        <div class="callout callout-info">
                            <strong><i class="fas fa-info"></i> Note:</strong>This page has been enhanced for printing. Click the print button at the bottom of the offer to test.
                        </div>


                        <!-- Main content -->
                        <div class="offer p-3 mb-3">
                            <!-- title row -->
                            <div class="row">
                                <div class="col-12">
                                    <h4>
                                        <i class="fas fa-globe"></i> Constructor
                                        <small class="float-right">Date: {{ date('d:m:Y', strtotime($offer->created_at)) }}</small>
                                    </h4>
                                </div>
                                <!-- /.col -->
                            </div>
                            ...
                            <div class="row offer-info">
                                <div class="col-sm-4 offer-col">
                                   <h2>*offer details: </h2>
                                    <address>
                                        <img class="img-fluid mb-2 rounded-circle" style="width: 150px; height: 150px;" src="{{(! empty($offer->Company->logo)) ? asset('attachments/offer/'.$offer->Company->id.'/'.$offer->Company->logo ) : asset('admin/dist/img/no_image.jpg') }}" alt="offer image">

                                        <br>
                                        <strong>{{$offer->Company->name ?? "Company name"}}, {{$offer->Company->specialization ?? "specialization Here"}} .</strong><br>
                                        Email: <strong>{{$offer->Company->email ?? "Email.com"}}</strong><br>
                                        <strong>{{$offer->Company->country ?? "country"}}, {{$offer->Company->governorate?? "governorate"}}</strong><br>
                                        <strong>{{$offer->Company->city ?? "city"}}, {{$offer->Company->zip_code ?? "C550"}}</strong><br>
                                        Phone: <strong>{{$offer->Company->phone ?? "phone"}}</strong>
                                    </address>
                                </div>
                                <!-- /.col -->
                            ...
                            <div class="col-sm-auto offer-col">
                                <h2>*offer details: </h2>
                                <address>
                                    {{--                                        <img class="img-fluid mb-2 rounded-circle" style="width: 150px; height: 150px;" src="{{(! empty($offer->image)) ? asset('attachments/offer/'.$offer->id.'/'.$offer->image ) : asset('admin/dist/img/no_image.jpg') }}" alt="offer image">--}}
                                    name:  <strong>{{$offer->name ?? "name"}}</strong><br>
                                    description: <strong>{{$offer->offer_description ?? "offer description..."}}</strong><br>
                                </address>
                            </div>
                            ...
                            <div class="row expandable-body" >
                                @foreach($offer->attachments()->get() as $offers_attachements)
                                    @foreach($offers_attachements->Images()->get() as $images)
                                        <img class="img-fluid mb-2 " style=" padding: 5px;margin-left: 10px; width: 150px; height: 150px;" src="{{(! empty($images->image_path)) ? asset('attachments/images/offer/'.$offer->id.'/'.$images->attachment_id.'/'.$images->image_path ) : asset('admin/dist/img/no_image.jpg') }}" alt="image">
                                    @endforeach
                                @endforeach
                            </div>
                            ...
                            {{--                                <hr>--}}
                            {{--                                @foreach($offer->attachments()->get() as $offers_attachements)--}}
                            {{--                                    @foreach($offers_attachements->Documents()->get() as $documents)--}}
                            {{--                                        <div style="position: relative; display: inline-block;">--}}
                            {{--                                            <embed src="{{ (!empty($documents->document)) ? asset('attachments/documents/offer/'.$offer->id.'/'.$documents->attachment_id.'/'.$documents->document) : asset('admin/dist/img/no_image.jpg') }}" type="application/pdf" style="padding: 5px; margin-left: 10px; width: 180px; height: 150px;" />--}}
                            {{--                                            <a href="{{ (!empty($documents->document)) ? asset('attachments/documents/offer/'.$offer->id.'/'.$documents->attachment_id.'/'.$documents->document) : '#' }}" download="document.pdf" style="position: absolute; bottom: 0; left: 50%; transform: translateX(-50%); margin-top: 5px;">Download</a>--}}
                            {{--                                        </div>--}}
                            {{--                                    @endforeach--}}
                            {{--                                @endforeach--}}
                            {{--                            </div>--}}
                            ...
                            <div class="row">
                                <div class="col-12">
                                    <div class="card">
                                        ...
                                        <!-- /.card-header -->
                                        <div class="card-body">
                                            <table id="example1" class="table table-bordered table-soffered">
                                                <thead>
                                                <tr>
                                                    <th>#</th>
                                                    <th>Name</th>
...
{{--                                                        <td>{{$invoice->offer->first_name}}</td>--}}
...
{{--                                                        <td>{{$quote->offer->first_name}}</td>--}}
                                                        <td>{{mb_substr($quote->offer->name,0,30). '...'}}</td>
                                                        <td>
...
{{--                                                        </td>--}}
                            ...
                            <div class="row">
                                ...
                                <!-- /.col -->
                                {{--                                <div class="col-6">--}}
                                {{--                                    <p class="lead">Amount Due {{$offer->issued_date}}</p>--}}
                                ...
                                {{--                                            </tr>--}}
                                {{--                                            <tr>--}}
                                {{--                                                <th>Tax ({{$offer->tax->ratio}}%)</th>--}}
                                {{--                                                <td>${{$tax}}</td>--}}
                                {{--                                            </tr>--}}
                                {{--                                            <tr>--}}
                                {{--                                                <th>Discount:</th>--}}
                                {{--                                                <td>${{$offer->discount->value}}</td>--}}
                                {{--                                            </tr>--}}
                                {{--                                            <tr>--}}
                                {{--                                                <th>Total:</th>--}}
                                {{--                                                <td>${{$offer->total}}</td>--}}
                            ...
                            <div class="row no-print">
                                <div class="col-12">
                                    {{--                                    <a href="offer-print.html" rel="noopener" target="_blank" class="btn btn-default"><i class="fas fa-print"></i> Print</a>--}}
