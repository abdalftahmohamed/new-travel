                ...
                <div class="col-12">
                    <div class="card">
                        ...
                        <div class="callout callout-info">
                            <strong><i class="fas fa-info"></i> Note:</strong>This page has been enhanced for printing. Click the print button at the bottom of the client to test.
                        </div>


                        <!-- Main content -->
                        <div class="client p-3 mb-3">
                            <!-- title row -->
                            <div class="row">
                                <div class="col-12">
                                    <h4>
                                        <i class="fas fa-globe"></i> Constructor
                                        <small class="float-right">Date: {{ date('d:m:Y', strtotime($client->created_at)) }}</small>
                                    </h4>
                                </div>
                                <!-- /.col -->
                            </div>
                            ...
                            <div class="row client-info">
                                <div class="col-sm-4 client-col">
                                   <h2>*client details: </h2>
                                    <address>
                                        <img class="img-fluid mb-2 rounded-circle" style="width: 150px; height: 150px;" src="{{(! empty($client->Company->logo)) ? asset('attachments/client/'.$client->Company->id.'/'.$client->Company->logo ) : asset('admin/dist/img/no_image.jpg') }}" alt="client image">

                                        <br>
                                        <strong>{{$client->Company->name ?? "Company name"}}, {{$client->Company->specialization ?? "specialization Here"}} .</strong><br>
                                        Email: <strong>{{$client->Company->email ?? "Email.com"}}</strong><br>
                                        <strong>{{$client->Company->country ?? "country"}}, {{$client->Company->governorate?? "governorate"}}</strong><br>
                                        <strong>{{$client->Company->city ?? "city"}}, {{$client->Company->zip_code ?? "C550"}}</strong><br>
                                        Phone: <strong>{{$client->Company->phone ?? "phone"}}</strong>
                                    </address>
                                </div>
                                <!-- /.col -->
                            ...
                            <div class="col-sm-auto client-col">
                                <h2>*client details: </h2>
                                <address>
                                    {{--                                        <img class="img-fluid mb-2 rounded-circle" style="width: 150px; height: 150px;" src="{{(! empty($client->image)) ? asset('attachments/client/'.$client->id.'/'.$client->image ) : asset('admin/dist/img/no_image.jpg') }}" alt="client image">--}}
                                    name:  <strong>{{$client->client_name ?? "name"}}</strong><br>
                                    description: <strong>{{$client->client_description ?? "client description..."}}</strong><br>
                                </address>
                            </div>
                            ...
                            <div class="row expandable-body" >
                                @foreach($client->attachments()->get() as $clients_attachements)
                                    @foreach($clients_attachements->Images()->get() as $images)
                                        <img class="img-fluid mb-2 " style=" padding: 5px;margin-left: 10px; width: 150px; height: 150px;" src="{{(! empty($images->image_path)) ? asset('attachments/images/client/'.$client->id.'/'.$images->attachment_id.'/'.$images->image_path ) : asset('admin/dist/img/no_image.jpg') }}" alt="image">
                                    @endforeach
                                @endforeach
                            </div>
                            ...
                            {{--                                <hr>--}}
                            {{--                                @foreach($client->attachments()->get() as $clients_attachements)--}}
                            {{--                                    @foreach($clients_attachements->Documents()->get() as $documents)--}}
                            {{--                                        <div style="position: relative; display: inline-block;">--}}
                            {{--                                            <embed src="{{ (!empty($documents->document)) ? asset('attachments/documents/client/'.$client->id.'/'.$documents->attachment_id.'/'.$documents->document) : asset('admin/dist/img/no_image.jpg') }}" type="application/pdf" style="padding: 5px; margin-left: 10px; width: 180px; height: 150px;" />--}}
                            {{--                                            <a href="{{ (!empty($documents->document)) ? asset('attachments/documents/client/'.$client->id.'/'.$documents->attachment_id.'/'.$documents->document) : '#' }}" download="document.pdf" style="position: absolute; bottom: 0; left: 50%; transform: translateX(-50%); margin-top: 5px;">Download</a>--}}
                            {{--                                        </div>--}}
                            {{--                                    @endforeach--}}
                            {{--                                @endforeach--}}
                            {{--                            </div>--}}
                            ...
                            <div class="row">
                                <div class="col-12">
                                    <div class="card">
                                        ...
                                        <!-- /.card-header -->
                                        <div class="card-body">
                                            <table id="example1" class="table table-bordered table-scliented">
                                                <thead>
                                                <tr>
                                                    <th>#</th>
                                                    <th>Name</th>
...
{{--                                                        <td>{{$quote->client->first_name}}</td>--}}
                                                        <td>{{mb_substr($quote->client->client_name,0,30). '...'}}</td>
                                                        <td>
...
{{--                                                        </td>--}}
                            ...
                            <div class="row">
                                ...
                                <!-- /.col -->
                                {{--                                <div class="col-6">--}}
                                {{--                                    <p class="lead">Amount Due {{$client->issued_date}}</p>--}}
                                ...
                                {{--                                            </tr>--}}
                                {{--                                            <tr>--}}
                                {{--                                                <th>Tax ({{$client->tax->ratio}}%)</th>--}}
                                {{--                                                <td>${{$tax}}</td>--}}
                                {{--                                            </tr>--}}
                                {{--                                            <tr>--}}
                                {{--                                                <th>Discount:</th>--}}
                                {{--                                                <td>${{$client->discount->value}}</td>--}}
                                {{--                                            </tr>--}}
                                {{--                                            <tr>--}}
                                {{--                                                <th>Total:</th>--}}
                                {{--                                                <td>${{$client->total}}</td>--}}
                            ...
                            <div class="row no-print">
                                <div class="col-12">
                                    {{--                                    <a href="client-print.html" rel="noopener" target="_blank" class="btn btn-default"><i class="fas fa-print"></i> Print</a>--}}
