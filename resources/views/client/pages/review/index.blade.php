            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h4>
                                <a href="{{route('admin.review.create')}}"
                                   class="btn btn-primary waves-effect waves-light float-end mb-4">Create review</a>
                            </h4>
                        ...
                        <!-- /.card-header -->
                        <div class="card-body">
                            <table id="example1" class="table table-bordered table-sreviewed">
                                <thead>
                                <tr>
                                    <th>#</th>
                                    <th>review name</th>
...
{{--                                    <th>Show</th>--}}
                                ...
                                @foreach($reviews as $review)
                                    <tr>
                                        ...
                                        <td>{{$review->name}}</td>
                                        <td>{{$review->stars_numbers}}</td>
                                        <td>{{mb_substr($review->description,0,40). '...'}}</td>
                                        <td>{{$review->client->name}}</td>
                                        <td>{{$review->trip->name ?? null}}</td>
                                        <td>{{$review->blog->name ?? null}}</td>
                                        <td>
...
{{--                                        <td>--}}
{{--                                            <a class="dropdown-item" href="{{ route('client.review.show', $review->id) }}" style="display: flex;padding-top: 20px; justify-content: center; align-items: center;">--}}
                                                ...
                                                <div class="btn-group">
                                                    ...
                                                    <div class="dropdown-menu" role="menu">
                                                        <a class="dropdown-item"
                                                           href="{{route('client.review.edit', $review->id)}}">Edit</a>
                                                        <div class="dropdown-divider"></div>
                                                        <form action="{{route('client.review.destroy')}}" method="POST"
